...
/**
 * REST controller for managing {@link com.kobe.nucleus.domain.Produit}.
 */
@RestController
@RequestMapping("/api")
public class ProduitResource {
	...
	@GetMapping("/produits/criteria")
	public ResponseEntity<List<ProduitDTO>> getAllProduitsWithCriteria(ProduitCriteria produitCriteria,
			Pageable pageable) {
		log.debug("REST request to get a page of produitCriteria====================>>>>    {}", produitCriteria);
		Page<ProduitDTO> page = produitService.findAll(produitCriteria, pageable);
		HttpHeaders headers = PaginationUtil
				.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
		return ResponseEntity.ok().headers(headers).body(page.getContent());
	}
	  ...
	  @GetMapping("/produits/details/{id}")
	    public ResponseEntity<ProduitDTO> getProduitDetails(@PathVariable Long id) {
	        log.debug("REST request to get Produit : {}", id);
	        Optional<ProduitDTO> produitDTO = produitService.findOne(id);
	        return ResponseUtil.wrapOrNotFound(produitDTO);
	    }
