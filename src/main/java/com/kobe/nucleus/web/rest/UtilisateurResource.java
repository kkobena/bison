...
/**
 * REST controller for managing {@link com.kobe.nucleus.domain.Utilisateur}.
 */
@RestController
@RequestMapping("/api")
public class UtilisateurResource {
    ...
    public ResponseEntity<List<UtilisateurDTO>> getAllUtilisateurs(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("roleutilisateur-is-null".equals(filter)) {
            log.debug("REST request to get all Utilisateurs where roleUtilisateur is null");
            return new ResponseEntity<>(utilisateurService.findAllWhereRoleUtilisateurIsNull(),
                    HttpStatus.OK);
        }
