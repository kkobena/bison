...
import com.kobe.nucleus.config.KafkaProperties;
...
class NucleusKafkaResourceIT {
    ...
    void setup() {
        ...
        NucleusKafkaResource kafkaResource = new NucleusKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/nucleus-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/nucleus-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
